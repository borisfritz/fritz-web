...
def generate_website(content_dir_path, template_path, dest_dir_path, basepath):
    for item in os.listdir(content_dir_path):
        ...
        if os.path.isdir(c_item):
            ...
            generate_website(c_item, template_path, d_item, basepath)
        elif item.lower().endswith(".md"):
            h_name = os.path.splitext(item)[0] +".html"
            h_item = os.path.join(dest_dir_path, h_name)
            generate_page(c_item, template_path, h_item, basepath)
